...
function MockCallJson{
    ...
    if($asHashtable.IsPresent){
        Set-InvokeCommandMock -Alias $command -Command "Get-MockFileContentJson -filename $filename -asHashtable"
    } else {
...
function Get-MockFileContentJson{
    ...
    $content = Get-MockFileContent -fileName $filename | ConvertFrom-Json -AsHashtable:$AsHashtable
